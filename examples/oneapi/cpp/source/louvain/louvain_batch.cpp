...
int main(int argc, char** argv) {
    ...
    const auto louvain_desc =
        descriptor<float, method::louvain, task::non_overlapping_communities, std::allocator<char>>(
            alloc).set_resolution(1).set_tolerance(2).set_max_iteration_count(3);
    // compute louvain
    const auto louvain_paths = dal::preview::community_detection(louvain_desc, my_graph);
